...
class lane_detection():
    def __init__(self):
        ...
        rospy.init_node('CvBridge_node', anonymous=False)
    ...
    def color_filter(self, image):
        ...
        lower = np.array([12, 12, 40]) #darkest detection
        upper = np.array([255, 255, 255])

        white_mask = cv2.inRange(hls, lower, upper)
        masked = cv2.bitwise_and(image, image, mask = white_mask)

        #filtering direction sign
        sign_region_mask = np.zeros_like(white_mask)
        height, width = image.shape[:2]
        sign_region = np.array([[(width // 2 - 50, height // 2), (width // 2 + 50, height // 2), (width // 2 + 50, height), (width // 2 - 50, height)]])
        cv2.fillPoly(sign_region_mask, sign_region, 255)
        masked = cv2.bitwise_and(masked, masked, mask=cv2.bitwise_not(sign_region_mask))

        return masked
    
    def warpping(self, image):
        
        source = np.float32([[230, 260], [400, 260], [110, 400], [520, 400]]) #setting ROI
    ...
    def slide_window_search(self, binary_warped, left_current, right_current):
        ...
        minpix = 50
        margin = 50 #sliding window margin(width)
        left_lane = []
        right_lane = []
        color = [0, 255, 0]
        thickness = 2
        ...
        for w in range(nwindows):
            ...
            cv2.moveWindow('sliding window', 0, 600)
    ...
    def main(self):
        if self.image is not None:
            ...
            blurred_img = cv2.GaussianBlur(warpped_img, (0, 0), 1) #reducing noise

            # BEV 필터링
            w_f_img = self.color_filter(blurred_img)
            cv2.namedWindow('Color filter')
            cv2.moveWindow('Color filter', 0, 0)
            cv2.imshow('Color filter', w_f_img)

            # BEV threshold
            _gray = cv2.cvtColor(w_f_img, cv2.COLOR_BGR2GRAY)
            ret, thresh = cv2.threshold(_gray, 190, 255, cv2.THRESH_BINARY) #reducing unnecessary detection
            cv2.namedWindow('threshold')
            cv2.moveWindow('threshold', 450, 0)
