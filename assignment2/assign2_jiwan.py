...
class Lanedetect():
    ...
    def warpping(self, image):
        ...
        destination = np.float32([[0, 0], [240, 0], [0, 480], [240, 480]])
        transform_matrix = cv2.getPerspectiveTransform(source, destination)
        minv = cv2.getPerspectiveTransform(destination, source)
        _image = cv2.warpPerspective(image, transform_matrix, (240,480))
        return _image, minv
    ...
    def main(self):
        if self.img is not None: 
            ...
            warpped_img, minverse = self.warpping(self.img)
            ...
            # 원본 이미지에 라인 넣기
            result, newwarp, color_warp, pts_left_list, pts_right_list = self.draw_lane_lines(self.img, thresh, minverse, draw_info)
